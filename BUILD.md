...
### מירור הבלוג של רדיס (`content/blog/`)

הרבה ממה שרדיס מפרסמת על הארכיטקטורה של עצמה קיים רק בבלוג, ולכן הוא ממורר פנימה כתוכן
Hugo. **התוכן מקובע בגיט** — כמו סנכרון הדוקס של RedisVL — כדי שהבנייה תישאר דטרמיניסטית
ולא תלויה ב-redis.io ברגע הבנייה.

```bash
make blog          # או: python3 -m build.blog_mirror
git add content/blog static/images/blog
```

הסקריפט מושך מה-dataset הציבורי של Sanity שמאחורי redis.io, ממיר Portable Text ל-markdown,
ומוריד את התמונות. הוא אידמפוטנטי: תמונה שכבר על הדיסק לא נמשכת שוב, אז ריצה חוזרת אחרי
הפסקה משלימה רק את מה שחסר. פוסט שנמחק במקור נמחק גם כאן.

| | |
|---|---|
| פוסטים | ~1,108 קבצי markdown, ~8.8 MB |
| תמונות | ~2,900 קבצים. רסטר מומר ל-WebP (‏82 KB → 34 KB בממוצע); SVG ו-GIF עוברים כמו שהם |
...
- קישורי הבלוג בתוך הדוקס מנותבים ל-`/blog/` ע"י `sed` על `content/**/*.md`, באותו מקום
  שבו כבר משוכתבים קישורי `redis.io/docs/latest/`. ארבעה דומיינים ולא אחד: מתוך 184
  הקישורים, 83 משתמשים ב-`redis.com` ו-16 ב-`redislabs.com`.
- בניות הגרסאות מוחקות את `content/blog` לפני Hugo. כל ריצת גרסה שומרת רק את
  `public/<product>/<version>/`, אז רינדור 1,100 פוסטים שם הוא עבודה שנזרקת 28 פעמים.
- `static/images/blog` מוחרג ממפתח ה-cache של בניות הגרסאות בלבד. בלי זה כל סנכרון בלוג
  היה מבטל את 29 ה-caches ובונה מחדש את כל המטריצה בשביל תמונות שאף עמוד גרסה לא מציג.
  במפתח של `latest` הוא **כן** נכלל, כי שם התמונות באמת נשלחות.

> **החיפוש מקבל את הבלוג בחינם.** הפוסטים הם עמודי Hugo, אז `build/generate_ndjson.py`
> אוסף אותם ל-`docs.ndjson` כמו כל עמוד אחר, ושירות החיפוש מתייג אותם `source=blog` לפי
> הנתיב. אין שלב נוסף.
