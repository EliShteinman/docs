...
### מירור תוכן מ-redis.io (`content/blog/`, `content/technology/`)

הרבה ממה שרדיס מפרסמת על הארכיטקטורה של עצמה קיים מחוץ לדוקס — בבלוג, ובמדריכי
`/technology/`. שניהם ממוררים פנימה כתוכן Hugo. **התוכן מקובע בגיט** — כמו סנכרון הדוקס של
RedisVL — כדי שהבנייה תישאר דטרמיניסטית ולא תלויה ב-redis.io ברגע הבנייה.

```bash
make mirror                                  # הכל
python3 -m build.site_mirror --only blog     # או רק אחד מהם
python3 -m build.site_mirror --only technology
git add content/blog content/technology static/images/site-mirror
```

הסקריפט מושך מה-dataset הציבורי של Sanity שמאחורי redis.io, ממיר ל-markdown, ומוריד את
התמונות. הוא אידמפוטנטי: תמונה שכבר על הדיסק לא נמשכת שוב. מסמך שנמחק במקור נמחק גם כאן.

| | |
|---|---|
| בלוג | 1,109 פוסטים ב-`/blog/<slug>/`, ~12 MB markdown. מוסתרים מהסרגל הצדדי (`hidden: true`) — 1,100 ערכים היו קוברים את הדוקס |
| technology | 10 מדריכים ב-`/technology/<slug>/`. **לא** מוסתרים — 11 ערכים הם סקשן שאפשר לנווט בו |
| תמונות | ~2,700 קבצים ב-`static/images/site-mirror/`. רסטר מומר ל-WebP (‏82 KB → 34 KB בממוצע); SVG ו-GIF עוברים כמו שהם |
| מתוכם GIF | 43 קבצים, **162 MB** — הקלטות מסך שההנפשה בהן היא התוכן. Sanity משטחת GIF לפריים בודד בכל המרה, אז השארתם כמות שהם היא התנאי לשמור אותן |

**שני מבנים, צינור אחד.** פוסט בלוג הוא שדה Portable Text אחד. עמוד `/technology/` הוא רשימת
סקשנים של page-builder שהפרוזה מפוזרת ביניהם — `build/site_mirror/pages.py` קורא את התשעה
סוגי הסקשן שהעמודים האלה משתמשים בהם בפועל. סוג לא מוכר לא תורם כלום ומדווח כאזהרה: העמוד
מאבד בלוק במקום לקבל בלוק שבור.

**מה קורה בבנייה, בשני הפייפליינים:**

- קישורי בלוג, `/technology/` ופקודות מנותבים פנימה ע"י `sed` על `content/**/*.md`, באותו
  מקום שבו כבר משוכתבים קישורי `redis.io/docs/latest/`. הבלוג דורש ארבעה דומיינים: מתוך 184
  הקישורים, 83 משתמשים ב-`redis.com` ו-16 ב-`redislabs.com`.
- בניות הגרסאות מוחקות את `content/blog` ו-`content/technology` לפני Hugo. כל ריצת גרסה
  שומרת רק את `public/<product>/<version>/`, אז רינדור התוכן הזה שם נזרק 28 פעמים.
- `static/images/site-mirror` מוחרג ממפתח ה-cache של בניות הגרסאות בלבד. בלי זה כל סנכרון
  היה מבטל את 29 ה-caches. במפתח של `latest` הוא **כן** נכלל, כי שם התמונות באמת נשלחות.

> **החיפוש מקבל את זה בחינם.** הכל עמודי Hugo, אז `build/generate_ndjson.py` אוסף אותם
> ל-`docs.ndjson` כמו כל עמוד אחר. פוסטי בלוג מתויגים `source=blog` לפי הנתיב ומקובצים
> בנפרד בחלון החיפוש; מדריכי `/technology/` מתויגים `docs`, כי זה מה שהם.
