...
> **TL;DR:**
>
> Register a `StackExchange.Redis` `ConnectionMultiplexer` in your ASP.NET Core app, then use `StringGetAsync` / `StringSetAsync` to cache API responses as JSON strings with an expiration. On a cache hit you skip the external call entirely, cutting response times from ~1 s to under 5 ms.
...
- How to connect an ASP.NET Core project to Redis using StackExchange.Redis
- How to cache REST API responses as JSON strings in Redis
- How to set a time-to-live (TTL) so cached data expires automatically
- How to measure the performance difference between cached and uncached requests
...
For our example, we will use the US National Weather Service's (NWS) Weather API - which is free and requires no authentication beyond a user-agent. We will build an API to get a weather forecast based on latitude and longitude using ASP.NET Core. If you are new to using Redis with .NET, check out the [Redis OM .NET getting started tutorial](/tutorials/redis-om-dotnet-getting-started/) first.
...
## How do you add a Redis cache to an ASP.NET Core app?
...
## What data structures hold the cached results?
...
## How do you inject Redis into the controller?
...
## How do you query the weather API?
...
## How does the cache-aside pattern work in the controller action?
...
Now that you have basic API caching working, explore these related tutorials:

- [Redis Streams in .NET](/tutorials/develop/dotnet/streams/stream-basics/) – learn how to use Redis Streams for event processing in your .NET applications.
- [Rate Limiting in .NET with Redis](/tutorials/rate-limiting-in-dotnet-with-redis/) – protect your APIs from overuse by adding Redis-backed rate limiting.
- [Redis OM .NET Getting Started](/tutorials/redis-om-dotnet-getting-started/) – simplify data persistence in Redis using the Redis OM .NET library.

## Resources

- Source code for this demo is located in [GitHub](https://github.com/redis-developer/basic-aspnetcore-api-caching)
- More documentation for the StackExchange.Redis library is located on its [docs site](https://stackexchange.github.io/StackExchange.Redis/)
