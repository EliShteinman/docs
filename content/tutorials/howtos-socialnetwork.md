---
title: "How to Build a Social Network Application using Redis and NodeJS"
...
> **TL;DR:**
>
> You can build a social network with Redis by storing user profiles as hashes, indexing skills and locations with Redis Search, and querying for matches using tag and geo filters. Pair this with a Node.js/Express backend to expose a REST API for creating users and finding complementary skill matches in real time.

![Illustration of a social network application connecting users through skill matching with Redis](/images/site-mirror/8a8adc21359fbef2396e3031ccf57d1c84a45492-2000x1331.webp)

## What you'll learn

- How to model user profiles as Redis hashes with tags and geo coordinates
- How to create a Redis Search index for efficient user matching
- How to query by tags (interests, expertises) and geo-radius filters
- How to build a Node.js/Express REST API backed by Redis
- How to containerize and run the full stack with Docker Compose
...
- A Redis instance (this tutorial uses the Redis Docker image)

---

In this tutorial you'll build a social network application using Redis and Node.js. This is the idea behind [Skillmarket](https://www.youtube.com/watch?v=18NPKZy28cQ).

The goal of the application is to match users with complementary skills. It will allow users to register and provide some information about themselves, like location, areas of expertise and interests. Using search in Redis it will match two users who are geographically close, and have complementary areas of expertise and interests, e.g., one of them knows French and wants to learn Guitar and the other knows Guitar and wants to learn French.

The full source code of our application can be found in GitHub (note that we used some features like `FT.ADD` which is now [`FT.CREATE`](https://redis.io/docs/latest/commands/ft.create/)):
...
Refer to the [official tutorial](https://github.com/RediSearch/redisearch-getting-started) for more information about search in Redis.

## How do you set up Redis Search in a Docker container?
...
Finally, `redis:latest` tells docker to use the `latest` version of the [redis image](https://hub.docker.com/_/redis)
...
## How do you store user profiles as Redis hashes?
...
![Diagram showing how a Redis Search index maps hash fields to searchable schema for efficient querying](/images/site-mirror/e2e57f192fa764554e7010217e2d9ca0a5a465e2-1298x714.webp)
...
## How do you query Redis to match users by skills and location?

Here we can see the power of the search index, which allows us to query by [tags](https://redis.io/docs/latest/develop/ai/search-and-query/advanced-concepts/tags/) (we provide a list of values, such as interests, and it will return any user whose interests match at least one value in the list), and [Geo](https://redis.io/docs/latest/develop/ai/search-and-query/advanced-concepts/query_syntax/#geo-filters) (we can ask for users whose location is at a given radius in km from a point).
...
We use the [`FT.CREATE`](https://redis.io/commands/ft.create/) command to create a full text search index named `idx:users`. We specify `ON hash` to indicate that we're indexing the hash table, and provide `PREFIX 1 "users:"` to indicate that we should index any document whose key starts with the prefix "users:". Finally, we indicate the `SCHEMA` of the index by providing a list of fields to index, and their type.

Finally, we can query the index using the [`FT.SEARCH`](https://redis.io/docs/latest/commands/ft.search/) command (see the [query syntax reference](https://redis.io/docs/latest/develop/ai/search-and-query/)):
...
## How do you build a Node.js backend for a Redis-powered social network?

After understanding how the index works, let's build a minimal backend API in Node.js that will allow us to create a user, and query for matching users.
...
async function createUserIndex() {
    client.ft_createAsync('idx:users', [
        'ON',
        'hash',
        'PREFIX',
        '1',
...
### How do you create, retrieve, and match users with a controller?
...
![Side-by-side comparison showing how user model fields map to Redis TAG and GEO storage formats](/images/site-mirror/81f82b891a17394e954b7960ff2c6993d40d7236-1292x612.webp)
...
### How do you expose the user matching API with Express?
...
### How do you run the full example with Docker Compose?

The code used in this tutorial can be found in the [GitHub repo](https://github.com/julianmateu/skillmarket-blogpost). The backend together with redis can be launched using docker compose:
...
Now that you've built a skill-matching social network with Redis, explore these related tutorials to expand your Redis application skills:

- **[Build a real-time chat application with Redis](/tutorials/howtos/chatapp/)** — Learn how to use Redis Pub/Sub for messaging and user presence in a chat app.
- **[Build a real-time leaderboard with Redis](/tutorials/howtos/leaderboard/)** — Use Redis Sorted Sets to implement ranking and score tracking.

You can also dive deeper into the features used in this tutorial:
...
- [node-redis client library](https://www.npmjs.com/package/redis)
