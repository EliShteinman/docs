...
> **TL;DR:**
>
> Build a session-based shopping cart by storing cart data in Redis hashes with `HSET cart:{cartId} product:{productId} {quantity}`. Use Node.js with Express for the backend, connect-redis for session management, and Vue.js for the frontend. Redis gives you sub-millisecond reads and writes so your add-to-cart operations stay fast at any scale.
...
[YouTube: https://www.youtube.com/embed/Fvjn5fuUxOU](https://www.youtube.com/embed/Fvjn5fuUxOU)

## What you'll learn

- How to store and manage e-commerce shopping cart data in Redis hashes
- How to use `express-session` with `connect-redis` for session-based cart management
- How to structure a Node.js Express backend with routes, controllers, and services
- How to build a Vue.js frontend that communicates with the cart API
- How to use Redis commands like `HSET`, `HGETALL`, `HINCRBY`, and `HDEL` for cart operations

## Prerequisites

- [Node.js](/tutorials/develop/node/gettingstarted/) v12.9.0 or later (this tutorial uses v15.5.0)
- NPM v6.14.8 or later
- A running Redis instance (local or [Redis Cloud](https://redis.io/try-free/))
- Docker and Docker Compose (optional, for running Redis locally)
- Basic familiarity with JavaScript, Express, and REST APIs
...
![Node.js and Redis shopping cart application architecture overview showing product listing and cart components](/images/site-mirror/9423c12231468ac7fa01ae07e89ba63a2d180a34-1276x492.webp)
...
### How do you run Redis for the shopping cart?
...
## How do you set up the Node.js backend server?

![Architecture diagram showing the Node.js Express backend connecting to Redis for session-based shopping cart storage](/images/site-mirror/3bd9ead7ba3723e32aee48e9dec7ffa9963a6135-1254x486.webp)
...
### How do you define the Express routes?
...
The web server skeleton already has a ./routes folder containing routes for the index, products and cart. (as shown under [https://github.com/redis-developer/basic-redis-shopping-chart-nodejs/tree/main/server/src/routes](https://github.com/redis-developer/basic-redis-shopping-chart-nodejs/tree/main/server/src/routes))
...
### How do the controllers handle cart requests?
...
class ProductIndexController {
    ...
    async index(req, res) {
        ...
        for (const product of products) {
            ...
            await this.redisClientService.jsonSet(
                `product:${id}`,
                '.',
...
### How does the Redis service layer work?
...
### How is the product data stored in Redis?
...
The cart data is stored in a Redis hash like:
...
### How does the overall request flow work?
...
## How do you set up the Vue.js frontend?
...
![Shopping cart application UI showing product listing with add-to-cart buttons and a cart sidebar displaying selected items and quantities](/images/site-mirror/120e1a90e205a6fdde01963f9bd9adb3ef125d9b-1382x726.webp)
...
- Learn how to [cache REST API responses using Redis and Node.js](/tutorials/how-to-cache-rest-api-responses-using-redis-and-nodejs/) to speed up your e-commerce backend even further
- Get started with [Node.js and Redis](/tutorials/develop/node/gettingstarted/) if you want a deeper introduction to the Redis client library
- Explore [Redis Cloud](https://redis.io/try-free/) for a fully managed Redis deployment that scales with your shopping cart traffic
