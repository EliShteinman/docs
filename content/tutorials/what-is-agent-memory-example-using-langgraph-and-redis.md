---
title: "What is Agent Memory? Example using LangGraph and Redis"
...
![Architecture diagram of AI agent memory showing short-term conversation history and long-term episodic and semantic memory stored in Redis](/images/site-mirror/da6582b49714fe7161b15d729c1212d8a8fdf38f-1244x596.webp)
...
1.  On cloud, deploy a [FREE instance of Redis in the cloud](https://redis.io/try-free/). Or, if you have your own version of Redis Software running, that works too!
...
![LangGraph agent nodes persisting conversation state to Redis using the RedisSaver checkpointer for short-term memory](/images/site-mirror/3ec96bb6de88d7dbd5a99df045818f933e7dd0ac-742x792.webp)
...
![Long-term agent memory architecture showing episodic and semantic memories stored and retrieved via vector search in Redis using RedisVL](/images/site-mirror/cf4ee5c2fcdecec8d587076103060c777a6e568b-902x472.webp)
...
> **NOTE:** If you're familiar with the [CoALA paper](https://arxiv.org/abs/2309.02427), the terms "episodic" and "semantic" here map to the same concepts in the paper. CoALA discusses a third type of memory, _procedural_. In our example, we consider logic encoded in Python in the agent codebase to be its procedural memory.
...
- **Build a full agent:** See how memory fits into a production-grade AI agent in the [Product Management Agent (PM Maestro) with Redis and LangGraph](/tutorials/howtos/product-management-agent-langgraph/) tutorial.
- **Add RAG to your agent:** Combine agent memory with retrieval-augmented generation by following the [RAG GenAI Chatbot with Redis](/tutorials/howtos/solutions/vector/gen-ai-chatbot/) tutorial.
- **Try LangGraph + Redis yourself:** Start with the [LangGraph Quickstart](https://langchain-ai.github.io/langgraph/tutorials/introduction/), then add the [Redis checkpointer](https://github.com/redis-developer/langgraph-redis) for fast, persistent memory.
- **Explore more AI recipes:** Browse the [Redis AI resources](https://github.com/redis-developer/redis-ai-resources) GitHub repo for code samples and notebooks.
- **Read the AI docs:** Check out the [Redis AI tutorials](/tutorials/vector/) for quickstarts on vector search, embeddings, and more.
- **Get started with Redis Cloud:** [Try Redis Cloud free](https://redis.io/try-free/) on AWS, Azure, or Google Cloud -- the easiest way to deploy Redis for your AI workloads.
