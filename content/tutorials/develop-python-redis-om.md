...
> **TL;DR:**
>
> Redis OM for Python is a declarative object mapper that lets you define Redis data models as Pydantic classes, validate input automatically, and query Redis using an ORM-like syntax — no raw commands needed.

[Redis OM for Python](https://github.com/redis/redis-om-python) makes it easy to model and query data in Redis using declarative models that will feel right at home to users of Peewee, SQLAlchemy, and the Django ORM. If you're familiar with Python ORMs like Peewee or SQLAlchemy, Redis OM provides a similar experience for Redis — with built-in Pydantic validation.

This tutorial will walk you through installing Redis OM for Python, creating your first model, and using it to save and validate data.

## What you'll learn

- How to install and configure Redis OM for Python
- How to define data models using `HashModel` and Pydantic type annotations
- How to validate data automatically, including strict validation
- How to save, retrieve, and query models in Redis
- How to create search indexes and run expressive queries with `find()`
...
> **NOTE**
>
> This tutorial will guide you through starting Redis locally, but the instructions will also work if Redis is running on a remote server.
...
We recommend the [redis](https://hub.docker.com/_/redis) image because it includes Redis modules that Redis OM can use to give you extra features. Later sections of this guide will provide more detail about these features.

You can also use the official Redis Docker image, which is hosted on [Docker Hub](https://hub.docker.com/_/redis).

> **NOTE**
>
> We'll talk about how to actually start Redis with Docker when we discuss running Redis later in this guide.

## Recommended: Redis Search and JSON

Redis OM relies on the Search and JSON support of Redis.
...
The easiest way to run these Redis modules during local development is to use the [redis](https://hub.docker.com/_/redis) Docker image.
...
#### **Docker with the** `**redis**` **image (recommended)**
...
## How do you define a model with Redis OM?
...
This means that you can use Pydantic field validations with your Redis OM models, which we'll cover later, when we talk about validation. But this also means you can use Redis OM models anywhere you would use a Pydantic model, like in your [FastAPI applications](/tutorials/develop/python/fastapi/).
...
## How does Redis OM validate data?

Redis OM uses [Pydantic](https://github.com/samuelcolvin/pydantic) to validate data based on the type annotations you assign to fields in a model class.
...
## How do you save a model to Redis?
...
## How can you examine your data in Redis?
...
> **NOTE**
>
> The naming of this method may be confusing. This is not the primary key, but is instead the Redis key for this model. For this reason, the method name may change.
...
## How do you retrieve a model by primary key?
...
## How do you query models with Redis OM?
...
# Now, if we use this model with a Redis deployment
...
## What are the differences between Redis OM and redis-py?

Redis OM builds on top of [redis-py](https://github.com/redis/redis-py) to provide a higher-level, declarative interface. While redis-py gives you direct access to Redis commands, Redis OM lets you work with Python classes and Pydantic models instead of raw keys and values. Use redis-py when you need fine-grained control over Redis commands; use Redis OM when you want an ORM-like experience with built-in validation and querying.

## Next steps

Now that you know the basics of working with Redis OM for Python, here are some ways to go further:

- **Build a web API:** Learn how to combine Redis OM with FastAPI in our [Using Redis with FastAPI](/tutorials/develop/python/fastapi/) tutorial.
- **Explore vector search:** See how to use Redis for similarity search in the [vector search tutorial](/tutorials/howtos/solutions/vector/getting-started-vector/).
- **Dive deeper into Redis OM:** Check out the [Redis OM for Python documentation](https://github.com/redis/redis-om-python) for advanced topics like `JsonModel`, embedded models, and async support.
