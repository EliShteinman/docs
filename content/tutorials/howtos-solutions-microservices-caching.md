...
- **[Cache prefetching](/tutorials/howtos/solutions/caching-architecture/cache-prefetching/)**: An entire product catalog can be pre-cached in Redis, and the application can perform any product query on Redis similar to the primary database.
...
![Cache-aside pattern architecture diagram showing the cache miss workflow: application requests data, backend checks Redis, data not found, query goes to primary database, result stored in Redis, then returned to application](/images/site-mirror/2ed93fd898249b50f969b594ba8aec15d88b74c6-843x1184.webp)
...
![Cache-aside pattern architecture diagram showing the cache hit workflow: application requests data, backend finds it in Redis, data returned directly without querying the primary database](/images/site-mirror/2ed93fd898249b50f969b594ba8aec15d88b74c6-843x1184.webp)
...
> **TIP**
>
> If you use **Redis Cloud** and a database that uses a JDBC driver, you can take advantage of [**Redis Smart Cache**](https://redis.io/blog/redis-smart-cache/), which lets you add caching to an application without changing the code. [**Click here to learn more!**](https://github.com/redis-field-engineering/redis-smart-cache)
...
![E-commerce microservices app dashboard built with Next.js showing the product catalog with search and filter functionality](/images/site-mirror/4f6a862752feff8ae6a946d92e5895906af325f3-1982x1500.webp)

`Shopping Cart`: Add products to the cart, then check out using the "Buy Now" button

![Shopping cart interface in the e-commerce microservices app showing selected products with a Buy Now checkout button](/images/site-mirror/9f9deeb554a22a16af87671c88606ac922b8e2e7-2000x1401.webp)

![E-commerce app checkout process showing order confirmation with Redis-cached product data](/images/site-mirror/776a16cd58782fb2ff513b9271b2b00b6a5d2d60-2000x1076.webp)
...
- **Add API gateway caching**: Combine query caching with [API gateway caching](/tutorials/howtos/solutions/microservices/api-gateway-caching) to cache responses at the gateway level and reduce load on individual microservices.
- **Try cache prefetching**: For data that you know will be needed, [cache prefetching](/tutorials/howtos/solutions/caching-architecture/cache-prefetching/) can pre-load your Redis cache so that first requests are also fast.
- **Explore other microservices patterns**: Learn how Redis supports [CQRS](/tutorials/howtos/solutions/microservices/cqrs) and [interservice communication](/tutorials/howtos/solutions/microservices/interservice-communication) in distributed architectures.
