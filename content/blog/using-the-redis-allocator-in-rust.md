...
linkTitle: "Using the Redis Allocator in Rust"
url: "/blog/using-the-redis-allocator-in-rust/"
description: "While developing redismodule-rs, the Rust API for writing Redis modules, I encountered the need to set up a custom memory allocator."
date: 2019-11-12
blogCategories:
- "Tech"
authors:
...
While developing [redismodule-rs](https://github.com/RedisLabsModules/redismodule-rs), the [Rust](https://www.rust-lang.org/) API for writing [Redis modules](https://redis.io/docs/latest/develop/reference/modules/), I encountered the need to set up a custom memory allocator.
...
After a bit of searching, we find that this is the way Rust does name mangling: Unlike in C, and similarly to C++, in Rust, multiple functions with the same name can coexist since there are various namespace mechanisms such as modules and traits to distinguish them. To generate unique symbols that are C-compatible, the compiler mangles these into long and ugly unique names. To detangle these names back into the original, we can filter the output through [rustfilt](https://crates.io/crates/rustfilt).
