...
linkTitle: "If You Think Goto Is a Bad Idea, What Would You Say About Longjmp?"
url: "/blog/if-you-think-goto-is-a-bad-idea/"
description: "I honestly disagree with the conventional wisdom of never using a goto in your code. There are several situations where I find it to be not just convenient but good practice. The most common case..."
date: 2019-09-16
blogCategories:
- "Tech"
authors:
...
Recently, we wanted to introduce error reporting to handle failures while evaluating expressions. For example, evaluating the static expression toUpper(5) would fail as the toUpper function expects its argument to be a string. If this assumption isn’t met, toUpper should raise an exception:
...
Unfortunately, C doesn’t come with a built-in exception mechanism like many other high-level languages do.
...
We could have introduced a check for error within each function on our execution path, but by doing so, we would hurt performance (branch prediction) and overcomplicate our code with if(error) return error; logical constructs all over the place.

And so, the jump is the first option that comes to mind, but note jump can only jump into a location within the function it is called in.
...
Another idea we had was calling ExecutionPlan_Execute within a new thread, such that when an exception was thrown, we would simply terminate the thread and resume execution within the “parent” thread. This approach would have saved us the need to introduce extra logic or code branching:
...
This is the design we’ve introduced recently. In case you ever run a query which violates the assumptions of a called function, this mechanism will be used to report an error back.
