---
title: "Using Stunnel to Secure Redis"
linkTitle: "Using Stunnel to Secure Redis"
url: "/blog/stunnel-secure-redis-ssl/"
description: "What do you do if you want to secure access to your Redis? Plain password authentication (i.e. the AUTH command) only gets you so far and in some cases you need something a little stronger. There..."
date: 2014-05-13
blogCategories:
- "Tech"
authors:
...
What do you do if you want to secure access to your Redis? Plain password authentication (i.e. the [AUTH](https://redis.io/commands/auth) command) only gets you so far and in some cases you need something a little stronger. There are several ways you could go about that, such as firewalling your Redis or using [spiped](https://groups.google.com/d/msg/redis-db/0ZDZapPZlLw/mpN8nWbnviAJ), but (post-[Heartbleed](/blog/security-notice-cve-2014-0160-heartbleed-openssl-vulnerability)) SSL is still one of my favorites. The following article explains how to set up a secure (read authenticated and encrypted) communication channel between your Redis client and server using stunnel. Naturally, this is the Do-It-Yourself hardcore approach, so if you want to have Redis and SSL without the heavy lifting, check out our plans and [Redis clients that natively support SSL](/blog/secure-redis-ssl-added-to-redsmin-and-clients).

— Itamar

*The following post had originally appeared on *[*Benjamin Cane’s Blog*](https://bencane.com/)* on February 18th, 2014 at *[*Sending redis traffic through an SSL tunnel with stunnel*](https://bencane.com/2014/02/18/sending-redis-traffic-through-an-ssl-tunnel-with-stunnel/)*.*
...
First we will create a private key, I am using openssl to create a 4096 bit RSA key. In my example I am using 4096 bit key as it [adds more security](https://danielpocock.com/rsa-key-sizes-2048-or-4096-bits) than a 1024 or 2048 bit key.
...
By default redis listens to the localhost IP 127.0.0.1 on port 6379Our configuration has stunnel accept connections on the external IP and forward the connections to the redis instance listening on 127.0.0.1:6379
...
Benjamin Cane, a.k.a. [@madflogo](https://twitter.com/madflojo), is the founder of CloudRoutes and a Solutions Architect working on High & Continuous Availability systems in the financial services industry. He has been Administrating Linux and Unix systems for about 10+ years. You might find him writing and sharing stories about Linux, Unix, Python, DevOps, and other various Sysadmin-like things.

Visit [Redis](/) for more on Redis technologies.
